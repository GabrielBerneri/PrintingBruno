...
const Cart = {
    ...
    getTransferDiscountEligibleSubtotal() {
        return this.getItems().reduce((sum, item) => sum + (item.transfer_discount > 0 ? item.price * item.quantity : 0), 0);
    },

    // Cada item puede tener su propio porcentaje de descuento
    getTransferDiscountAmount() {
        const total = this.getItems().reduce((sum, item) => {
            if (item.transfer_discount <= 0) return sum;
            return sum + (item.price * item.quantity * item.transfer_discount / 100);
        }, 0);
        ...
        <form id="checkoutForm">
          ...
          <div class="checkout-summary">
            ...
            <div class="checkout-summary-row" id="checkoutDiscountRow" style="margin-top: 0.5rem; display: none; color: #22c55e; font-weight: 600;">
              <span>Descuento (productos seleccionados)</span>
