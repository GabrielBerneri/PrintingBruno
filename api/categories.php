 ...
 * GET    → list all
 * POST   → create { name, slug?, sort_order?, active? }
 * PUT?id → update
 * DELETE?id → delete (only if no products use it)
 */
...
try {
    ...
    if ($method === 'POST') {
        $name  = trim($body['name'] ?? '');
        if ($name === '') { http_response_code(400); echo json_encode(['success'=>false,'error'=>'El nombre es obligatorio']); exit; }
    ...
    if ($method === 'DELETE') {
        ...
        $row->execute([$id]);
        $cat = $row->fetch();
        if (!$cat) { http_response_code(404); echo json_encode(['success'=>false,'error'=>'Categoría no encontrada']); exit; }
