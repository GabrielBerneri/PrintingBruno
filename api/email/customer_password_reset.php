...
function sendCustomerPasswordResetEmail(array $customer, string $token): bool
{
    $db = getDB();
    ...
    $resetUrl = rtrim(SITE_URL, '/') . '/mi-cuenta.php?reset=' . rawurlencode($token);
    $subject = 'PrintingBruno - Recuperación de contraseña';
...
HTML;
    ...
    try {
        ...
        $mail->Body = $html;
        $mail->AltBody = "Hola {$recipientName}. Restablecé tu contraseña con este enlace: {$resetUrl}";
        $mail->send();
        ...
        error_log('customer_password_reset_email_error: ' . $e->getMessage());
        try {
            ...
            error_log('customer_password_reset_email_log_failure: ' . $logError->getMessage());
        }
        return false;
    }
}
